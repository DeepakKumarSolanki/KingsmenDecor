...
const Header = ({ sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <header className="sticky top-0 z-999 flex w-full bg-white drop-shadow-1 dark:bg-boxdark dark:drop-shadow-none">
     ...
     {/* Hamburger Menu for Small and Medium Screens Only */}
<div className="flex items-center gap-2 sm:gap-4 2xl:hidden"> {/* This will hide the hamburger on screens larger than 'md' */}
  <button
  ...
  >
    <span className="relative block h-5.5 w-5.5 cursor-pointer">
      <span className="absolute right-0 h-full w-full">
        ...
        <span
          className={`relative block h-0.5 w-full rounded-sm bg-black duration-200 ease-in-out dark:bg-white ${
            sidebarOpen && 'rotate-45 top-3'
          }`}
        ></span>
        <span
          className={`relative block h-0.5 w-full rounded-sm bg-black mt-2 duration-200 ease-in-out dark:bg-white ${
            sidebarOpen && 'opacity-0'
          }`}
        ></span>
        <span
          className={`relative block h-0.5 w-full rounded-sm bg-black mt-2 duration-200 ease-in-out dark:bg-white ${
            sidebarOpen && '-rotate-45 bottom-2'
          }`}
        ></span>
      </span>
    </span>
